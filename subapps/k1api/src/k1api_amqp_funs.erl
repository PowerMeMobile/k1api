...
-export([queue_declare/2, queue_declare/3, queue_declare/5]).
...
-spec queue_declare(pid(), binary()) -> ok | {error, any()}.
queue_declare(Chan, Queue) ->
	queue_declare(Chan, Queue, []).

-spec queue_declare(pid(), binary(), [{atom(), term()}]) ->
                    'ok' | {'error', any()}.
queue_declare(Chan, Queue, Props) ->
...
parse_opts(Opts) ->
	DefaultPropList =
		case application:get_env(amqp_props) of
			{ok, Value} -> Value;
			undefined -> []
		end,
